...
n_traces = 128
n_times = 768
true_data = np.fromfile("data_1_blended_data_2.bin", np.float32).reshape(
    n_traces, n_times
)
out_data = np.fromfile("out/data_1_blended_data_2.bin", np.float32).reshape(
    n_traces, n_times
)

_, ax = plt.subplots(1, 3, figsize=(15, 7), sharex=True, sharey=True)
