...
n_traces = 128
n_times = 512
true_data = np.fromfile("data_1_true_data.bin", np.float32).reshape(
    n_traces, n_times
)
in_data = np.fromfile("data_1_blended_data.bin", np.float32).reshape(
    n_traces, n_times
)
out_data = np.fromfile("out/data_1_deblended_data.bin", np.float32).reshape(
    n_traces, n_times
)
